...
void setup() {
  ...
  void loop() {    
            ...
            // read analog input values of A0, A1, and A2               
                  float  a0Val =  analogRead(A0) ;                // Reads voltage output of PV System
                    float a1Val =  analogRead(A1) ;             //- offsetVoltage ;  /// Reads Voltage output from Current Sensor (PV End)
                    float a2Val =  analogRead(A2) ;             // Reads Voltage output from Current Sensor (Load End)
                  float  current  = (((a0Val  - a2Val) * 5.0) /1023) /0.02 ;
                    power [i] = ( a0Val  * 10.0 * 5.0 / 1023) * current ;
          ...
          // display the input values on the LCD display
              ...
              lcd.setCursor(9,1);
          //  lcd.print("PV V=");
              lcd.print((a0Val  * 5.0 / 1023.0 ) * 10.0);
              lcd.setCursor(9,2);
              lcd.print(current );
              lcd.setCursor(9,3);
              lcd.print(power [i]);
